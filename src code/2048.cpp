...
public:
	...
	state select_best_move(const board& b) const {
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {
				...
				float estimate_all=0;
				//get all possible state after pop up
				int possible_state=0;
				for(int i=0;i<16;i++){
					if (after_b.at(i)==0){
						//is blank
						possible_state+=1;
						...
						//random as 2
						temp.set(i,1);
						estimate_all+=0.9*estimate(temp);
						//random as 4
						temp.set(i,2);
						estimate_all+=0.1*estimate(temp);
						//info<<possible_state<<" est: "<<estimate_all<<std::endl;
					}
				}
				//info<<"estimate_all:"<<estimate_all<<"  possible_S: "<<possible_state<<"\n";
				if(possible_state==0){
					// no blank
					estimate_all=estimate(after_b);
				}else{
					estimate_all/=possible_state;
				}
				
				
				move->set_value(move->reward() + estimate_all);
	...
	void make_statistic(size_t n, const board& b, int score, int unit = 1000) {
		...
		if (n % unit == 0) { // show the training process
			...
			std::ofstream ofs;
			ofs.open("100W_training_score.txt",std::ios::app);
			ofs<<score<<","<<mean<<","<<max<<"\n";
			ofs.close();
...
int main(int argc, const char* argv[]) {
	...
	path.reserve(1000000);
	//clear record file
	std::ofstream ofs;
	ofs.open("100W_training_score.txt",std::ios::trunc);
	ofs.close();

	ofs.open("100W_score_only.txt",std::ios::trunc);
	ofs.close();
	...
	for (size_t n = 1; n <= total; n++) {
		...
		ofs.open("100W_score_only.txt",std::ios::app);
		ofs<<score<<std::endl;
		ofs.close();
		...
		if(n%10000==0){
			tdl.save("./model/"+std::to_string(n));
		}
	}
